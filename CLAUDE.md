...
- **スタンドアロン版禁止**: Astroでの開発時は必ずプロジェクト統合版を使用する

### ページ構成・設計方針（重要）

#### standard-predictionsページ
- **対象レース**: 後半3レース（10R、11R、12R）専用
- **1R-9Rは対象外**: 前半レースは含まない設計
- **アコーディオン構成**: race-10r-accordion、race-11r-accordion、race-12r-accordion
- **統一デザイン**: グリーン系スキーム（rgba(16, 185, 129, 0.05)ベース）
- **JavaScript**: toggleRace10R()、toggleRace11R()、toggleRace12R()関数で制御

#### 予想サービス区分
- **プレミアム予想**: 全12レース対象（1R-12R全て）
- **無料予想**: メインレースのみ対象（最終レースの1つ前。1-12Rなら11R、1-10Rなら9R）

### AI助手とのコミュニケーション改善（重要）

#### 対話方針
- **もっと日本語で自然に対話する**
- 作業前に必ず「どのように進めますか？」と確認する
- 指示が不明確な場合は「〜ということでしょうか？」と具体的に聞き返す
- 推測で作業を開始しない
- ユーザーの意図を正確に理解してから実行する

#### 作業パターンの理解
- 「〜をベースに」= 既存ファイルをコピーして修正する
- 「〜から作って」= 既存ファイルを参考に新規作成する  
- 「〜を修正して」= 既存ファイルを直接編集する
...
## CMS自動化システム（Phase 3完了）

### 概要
レース結果更新を30分→3分に短縮する完全自動化システムを実装。

### システム構成
1. **データ分離**: `src/data/raceResults.json` で一元管理
2. **管理者パネル**: `src/pages/admin.astro` で直感的な入力インターface
3. **一括ペースト機能**: 既存のコピペワークフローをそのまま活用

### 管理者パネル機能
- **URL**: `http://localhost:4321/admin`
- **一括ペースト解析**: ドット区切り配当（1.430円）とカンマ区切り（1,330円）両対応
- **自動データ抽出**: 日付、競馬場、レース番号、的中判定、配当金額
- **JSON自動生成**: 統計計算（的中率、回収率）含む
- **ワンクリックコピー**: 生成JSONをクリップボードにコピー

### 更新手順
1. 管理者パネル（/admin）にアクセス
2. 一括ペースト欄にレース結果データを貼り付け
3. 「🔍 データ解析」ボタンをクリック
4. 「🚀 JSONデータ生成」ボタンをクリック
5. 「📋 JSONをコピー」でコピー
6. `src/data/raceResults.json` の `currentResults` セクションに貼り付け
...
### 技術仕様
- **正規表現**: `(\d[\d,.]*)円` でドット・カンマ区切り両対応
- **全角変換**: 全角数字（１Ｒ）から半角数字（1R）への自動変換
- **エラーハンドリング**: データ解析失敗時のアラート表示
- **レスポンシブ**: モバイル対応の管理画面

### 効果
- **更新時間**: 30分 → 3分（90%短縮）
- **エラーリスク**: ほぼゼロ（自動計算・検証）
- **操作性**: 既存ワークフロー維持で学習コスト無し