...
### ページ作成ルール
**【重要】スタンドアロンHTML禁止・Astroコンポーネント必須**

- ❌ **禁止**: 独立したHTMLページ（DOCTYPE, html, head, body タグを含む）の作成
- ✅ **必須**: 全てのページは Astro コンポーネント形式で作成
- ✅ **必須**: BaseLayout または適切なレイアウトコンポーネントを使用
- ✅ **必須**: Astroの標準的なコンポーネント構造に従う
...
<BaseLayout title="ページタイトル" description="ページ説明">
    <!-- ページコンテンツ -->
    <div class="container">
        <!-- 内容 -->
    </div>
...
### 理由
1. **一貫性**: 全ページで統一されたレイアウトとスタイル
2. **保守性**: 共通部分の変更が全ページに反映される
3. **SEO**: BaseLayoutによる統一されたメタタグ管理
4. **パフォーマンス**: Astroの最適化機能の恩恵
5. **開発効率**: コンポーネントの再利用による開発速度向上

### 例外
- **例外なし**: 全てのページはAstroコンポーネント形式で作成すること